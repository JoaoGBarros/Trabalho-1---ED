...
void EnviarPacotesDados(ListaR* r, ListaT* t, char* chave1, char* chave2, FILE* saida){
    ...
    int tam = 2;
    int* tamPointer = &tam;
    int* rots = (int*)malloc(tam * sizeof(int));
    ...
    if(EnlaceRoteador(rot1, rot2, rots, tam)){
...
void ImprimeNetMap(ListaR* listaRot, ListaT* listaTer, FILE* saida){
    CelulaT* terminal = RetornaProximaCelulaTerminal(listaTer);
    CelulaR* roteador;
    CelulaEnlaces* enlace;
    ...
    for(terminal; terminal != NULL; terminal = RetornaProximaCelulaTerminal(terminal)){
        ...
        for(roteador; roteador != NULL; roteador = RetornaProxCelulaRoteador(roteador)){
            if(ReoteadortornaIdR(RetornaRoteadorLista(roteador)) == RetornaIdRot(terminal)){
                fprintf(saida, " -- %s", RetornaNomeRoteador(RetornaRoteadorLista(roteador)));
            }
        }
        fprintf(saida, "; ");
        // Puta que pariu que bagunca do caralho
    }
    for(roteador = RetornaPrimeiraCelulaListaRoteador(listaRot); roteador != NULL; roteador = RetornaPrimeiraCelulaListaRoteador(listaRot)){
        enlace = RetornaEnlace(roteador);
        if(!enlace){
            fprintf(saida, "%s; ", RetornaNomeRoteador(RetornaRoteadorLista(roteador)));
        }
        for(enlace;enlace != NULL;enlace = RetornaProximoEnlace(enlace)){
            fprintf(saida, "%s -- %s; ", RetornaNomeRoteador(RetornaRoteadorLista(roteador), RetornaNomeRoteador(RetornaRoteadorLista(RetornaRoteadorCelulaEnlace(enlace)))));
        }
    }
    fprintf(saida, "}");
